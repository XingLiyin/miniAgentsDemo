...
读取 `skills/bng-hld-hardware-cfg/references/design_principles.md` 了解设计原则。
...
使用 `Read` 工具读取 `skills/bng-hld-hardware-cfg/references/procurement_replacement.xlsx`（以"旧BomCode"为键，查"新BomCode"和"新Description"）。
...
读取 `skills/bng-hld-hardware-cfg/outputs/output_template.md`，替换所有占位符后原样输出，不得增删任何内容：

- `{{ROOM_A_TABLE}}` → Step 1 结果
- `{{ROOM_B_TABLE}}` → Step 3 结果
- `{{CHANGE_LIST}}` → Step 4 结果（每条以 `- ` 开头）

生成完成后读取 `skills/bng-hld-hardware-cfg/checks/self_check.md` 进行自校验，不满足则重新推理生成。
