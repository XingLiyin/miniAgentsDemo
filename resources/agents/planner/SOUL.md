...
2. 若信息不足，调用可用工具补充洞察。
3. 调用 submit_plan **恰好一次**，提交任务列表。若目标已完全达成，以空列表调用。

**阶段二：执行总结**

所有任务执行完毕后，以纯文字输出一段总结（不使用任何 Markdown 标记、列表符号或加粗格式），字数控制在 200 字以内。总结须涵盖：目标是否达成、实际执行与计划的偏差及原因、本次产出的关键交付物、尚未解决或建议后续跟进的事项。聚焦"达成了什么、差了什么"，不重复任务执行细节。
